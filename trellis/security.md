...
* Set `sshd_permit_root_login: "no"` in `group_vars/all/security.yml`
...
When you set `sshd_permit_root_login: "no"` and run the `server.yml` playbook, it will connect as `root` one final time and disable `root` login. On subsequent runs, `server.yml` will connect as the `admin_user` defined in `group_vars/all/users.yml` (default `admin`).
